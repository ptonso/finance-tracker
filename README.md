...
Workflow:
A cada mês, você deverá solicitar o extrato da conta do nubank. Coloque-o em data/00_raw.
Em seguida, você realizará a limpeza automática, categorização semiautomática e reconciliação.
Quando seus dados estiverem preparados, o notebook 10_analyzer.ipynb criará os dashboards para você.

Detalhes:
**Categorizador**
Você precisará adicionar manualmente descrições no arquivo category_lookup.json. Esse é o seu ponto de partida para a categorização de transações.
...
**Reconciliação**
Você também precisará verificar no seu banco qual é o saldo de cada mês. Isso garante que seus dados estejam corretos.
